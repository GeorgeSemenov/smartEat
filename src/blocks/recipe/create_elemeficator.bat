...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/recipe%filename%.pug
  )> newFile.pug
  type recipe.pug >> newFile.pug
  move /Y newFile.pug  recipe.pug
  (
...
(
echo @import '%fileName%/recipe%fileName%';
)>>recipe.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=recipe
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=recipe%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/recipe%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
