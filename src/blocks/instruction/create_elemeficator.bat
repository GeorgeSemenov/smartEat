...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/instruction%filename%.pug
  )> newFile.pug
  type instruction.pug >> newFile.pug
  move /Y newFile.pug  instruction.pug
  (
...
(
echo @import '%fileName%/instruction%fileName%';
)>>instruction.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=instruction
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=instruction%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/instruction%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
