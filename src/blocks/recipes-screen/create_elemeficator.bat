...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/recipes-screen%filename%.pug
  )> newFile.pug
  type recipes-screen.pug >> newFile.pug
  move /Y newFile.pug  recipes-screen.pug
  (
...
(
echo @import '%fileName%/recipes-screen%fileName%';
)>>recipes-screen.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=recipes-screen
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=recipes-screen%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/recipes-screen%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
